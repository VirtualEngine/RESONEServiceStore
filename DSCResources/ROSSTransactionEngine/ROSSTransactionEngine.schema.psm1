configuration ROSSTransactionEngine {
<#
    .SYNOPSIS
        Installs the RES ONE Service Store Transaction Engine.
#>
    param (
    ...
    $resourceName = 'ROSSTransactionEngine';
    ...
    if (-not $IsLiteralPath) {
        ...
        switch ($Version.Major) {
            7 {
                $setup = 'RES-ITS-Transaction-Engine({0})-{1}.msi' -f $Architecture, $Version.ToString();
                $name = 'RES IT Store Transaction Engine 2014';
            }
            8 {
                $setup = 'RES-ONE-ServiceStore-2015-Transaction-Engine({0})-{1}.msi' -f $Architecture, $Version.ToString();
                $name = 'RES ONE Service Store 2015 Transaction Engine';
            }
            Default {
    ...
    if ($Ensure -eq 'Present') {
        xPackage $resourceName {
            ...
            ReturnCode = 0;
            InstalledCheckRegKey = 'Software\RES\ITStore\Transaction';
...
} #end configuration ROSSTransactionEngine
