configuration ROSSDatabase {
<#
    .SYNOPSIS
        Creates the RES ONE Service Store database using the legacy console.
    .NOTES
        This also installs the legacy console (Setup & Sync Tool).
#>
    param (
    ...
    if ($Ensure -eq 'Present') {
        xPackage $resourceName {
            ...
            ProductId = '';
            Path = $Path;
            Arguments = 'DBSERVER="{0}" DBNAME="{1}" DBUSER="{2}" DBPASSWORD="{3}" DBTYPE="MSSQL" CATALOGSERVICESPASSWORD="{4}" DBCREATE="yes" DBCREATEUSER="{5}" DBCREATEPASSWORD="{6}"' -f $DatabaseServer, $DatabaseName, $Credential.Username, $Credential.GetNetworkCredential().Password, $CatalogServicesCredential.GetNetworkCredential().Password, $SQLCredential.Username, $SQLCredential.GetNetworkCredential().Password;
            ReturnCode = 0;
...
} #end configuration ROSSDatabase
